...
- (void)updateIDCountDownButtonTitle {
    if (self.tempDurationOfCountDown == 0) {
        ...
        [self setBackgroundColor:kTabbarHighlightedColor];
        [self setTitle:@"重新获取" forState:UIControlStateNormal];
        [self.countDownTimer invalidate];
        
        if ([self.customDelegate respondsToSelector:@selector(countdownEndNotification:)]) {
            [self.customDelegate countdownEndNotification:YES];
        }
        ...
        [self setBackgroundColor:kButtonBgGrayColor];
        
        if ([self.customDelegate respondsToSelector:@selector(countdownEndNotification:)]) {
            [self.customDelegate countdownEndNotification:NO];
        }
    }
}
