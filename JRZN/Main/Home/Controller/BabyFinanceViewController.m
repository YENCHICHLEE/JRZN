...
#import "BabyFinanceViewController.h"
#import "FilterBaseTableViewCell.h"
#import "FilterModel.h"

@interface BabyFinanceViewController ()<UITableViewDataSource, UITableViewDelegate, FilterBaseTableViewCellDelegate>

@property (nonatomic, strong) UITableView *tableView;

@end

@implementation BabyFinanceViewController
...
- (void)loadData{
    ...
    NSArray *categoryNameArray = @[@"发行机构",
                                   @"提现方式"];
    
    NSArray *categoryDetailListArray = @[@[@"不限", @"互联网企业", @"银行", @"基金"],
                                         @[@"不限", @"实时", @"T+1"]
                                         ];
    ...
    if ([UserModel shareUserModelManager].babyFinanceArray.count) {
        
        selectButtonArray = [UserModel shareUserModelManager].babyFinanceArray;
    }else{
        selectButtonArray = [@[@"0", @"0", @"0", @"0"] mutableCopy];
        [UserModel shareUserModelManager].babyFinanceArray = selectButtonArray;
    }
...
- (void)didSelectedFilterButton:(UIButton *)button{
    ...
    [UserModel shareUserModelManager].babyFinanceArray[indexPath] = @(indexPathRow);
...
- (void)resetButtonClick{
    ...
    [UserModel shareUserModelManager].babyFinanceArray = [@[@"0", @"0", @"0", @"0"] mutableCopy];
